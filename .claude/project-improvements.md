...
#### 本番環境でのデータベース問題 (2025-06-23)
- **問題**: お知らせシステム実装後、本番環境で既存イベントが表示されなくなった
- **原因**: Announcementテーブルの外部キー制約が`onDelete: Cascade`で設定され、参照整合性エラーが発生
- **解決策**: 
  - `created_by`フィールドをオプショナル（`String?`）に変更
  - 外部キー制約を`onDelete: SetNull`に変更
  - スキーマ修正後にデータベースを適切にマイグレーション
- **重大な教訓**: 
  - **絶対に本番で`--force-reset`を使用しない**
  - 外部キー制約は慎重に設計（`Cascade`は危険、`SetNull`や`Restrict`を優先）
  - 新テーブル追加時は既存データとの互換性を十分検証
  - 本番デプロイ前に必ずステージング環境でテスト
  - データベース変更前は必ずバックアップを取得
